...
public class Sensors {
    ...
    public int GetNearestPowerPillDistance(Game game, MOVE move){
        ...
        //System.out.println("PowerpILL" + minDistance);
        return minDistance;
    }
...
//			            }
        ...
        int[] order = new int[4];
        for(int j=0;j<A.size();j++)
            order[j] = A.get(j).getValue();

        for(int j=0;j<4;j++)
            ans[num++] = (double)ghosts[order[j]].fst/MaxDis;

        for(int j=0;j<4;j++)
            ans[num++] = (double)ghosts[order[j]].snd;

        for(int j=0;j<4;j++)
            ans[num++] = (double)ghosts[order[j]].trd;

        for(int j=0;j<4;j++)
            ans[num++] = (double)ghosts[order[j]].fth;*/
		/*
        System.out.format("BB\n");
        for(int j=0;j<ans.length;j++){
            System.out.format("%f,",ans[j]);
        }
        System.out.format("\n");*/
        return ans;
    }
